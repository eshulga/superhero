...
class HeroesList extends Component {
  ...
  newHero = newHero => {
    if (Object.keys(newHero).length !== 0) {
      const heroExist = this.state.list.filter(
        item => item.name === newHero.name,
      );

      if (heroExist.length === 0)
        postHero({ ...newHero }).then(response => {
          this.setState({
  ...
  handleSearch = e => {
    ...
    const filteredList = list.filter(
      item => item.name.toLowerCase().indexOf(target.value.toLowerCase()) > -1,
    );
    this.setState({
